...
本ドキュメントは、自律型コーディングエージェント (Jules / Devin / Codex / Claude Code / GitHub Copilot / Cursor / Cline / Windsurf / Aider / Sweep / PR-Agent 等) が、公開 OSS リポジトリ [`genzouw/ubuntu-sandbox`](https://github.com/genzouw/ubuntu-sandbox) で作業し Pull Request を作成するときに **必ず守るべき規範** を定義します。
...
- [ ] LLM プロバイダの API キー / 従量課金 API キーを GitHub Secrets に追加していない。`GEMINI_API_KEY` / `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` 等を `secrets.*` から参照する記述が変更差分（新規ファイルだけでなく既存ファイルの変更を含む）に含まれていない。
...
**承認の効力**: リポジトリオーナーが Issue 上で明示的に承認した場合に限り、1.1 節の自動クローズ規定の適用対象外となります。承認を得て PR を作成する際は、承認元の Issue URL を PR 本文に **MUST** で明記してください。Issue での承認を得ていない状態で有料サービスを含む PR を提出した場合、内容の良し悪しにかかわらず自動的にクローズされます（承認は事前取得が前提であり、事後承認は認められません）。
...
本ポリシーは、本ファイルが配置されているリポジトリ ([`genzouw/ubuntu-sandbox`](https://github.com/genzouw/ubuntu-sandbox)) にのみ適用されます。
[@genzouw](https://github.com/genzouw) の他の公開リポジトリに同じ方針を適用したい場合は、当該リポジトリのルートにも本ファイル（または同等のポリシー）を個別に配置してください。リポジトリをまたいで自動的に本ポリシーが適用される仕組みは現時点では存在しません。
